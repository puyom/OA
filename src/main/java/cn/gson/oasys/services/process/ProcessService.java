   ...
   */
	...
	public List<Map<String,Object>> index2(Page<AubUser> page,User user){
		...
		for (int i = 0; i < prolist.size(); i++) {
			...
			Long processId = prolist.get(i).getProcessId();
			ProcessList processList = new ProcessList();
			processList.setProcessId(processId);
			Map<String, Object> result=new HashMap<>();
			if(budao.findByProId(processList)!=null){
				result.put("money",budao.findByProId(processList).getAllMoney());
			}
	...
	public void  index66(Model model,Long id){
		User lu=udao.findOne(id);//申请人
//		Pageable pa=new PageRequest(page, size);
		List<SystemTypeList> harrylist=tydao.findByTypeModel("aoa_process_list");
		//查看用户并分页
...
//		model.addAttribute("page", pageuser);
	...
	public void index55(ProcessList pro,String val,User lu,String name) throws IllegalStateException, IOException {
	...
	public void save2(Long proid,User u,Reviewed reviewed,ProcessList pro,User u2){
		Reviewed re=redao.findByProIdAndUserId(proid,u);
//		re.setAdvice(reviewed.getAdvice());
		re.setStatusId(reviewed.getStatusId());
		re.setReviewedTime(new Date());
